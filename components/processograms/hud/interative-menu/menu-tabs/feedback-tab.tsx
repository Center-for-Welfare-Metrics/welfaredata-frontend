...
const FeedbackTab = () => {
  ...
  const submitForm = (event: FormEvent<HTMLFormElement>) => {
    ...
    let real_description = `Email: ${email || "Not Provided"}<br/><br/>${
      email ? `NewsLetter: ${newsletter ? "Yes" : "No"}` : ""
    }<br/><br/>${
      detailed
        ? `-------- description --------<br/>${detailed}<br/><br/><br/><br/>`
        : ""
    }-------- path --------<br/>${path_log}`;
    ...
    feedback
      ...
      .catch(() => {
        toast.error(
          <div onClick={(e) => e.stopPropagation()}>
            I'm sorry but I could not receive your feedback. Try later :)
          </div>
        );
      })
      .finally(() => {
      ...
      <FeedBackForm>
        <form method="post" onSubmit={submitForm}>
          ...
          {EmailValidator.validate(email) && (
            <CheckContainer>
              <CheckButton ischecked={newsletter} htmlFor="newsletter">
                <div />
              </CheckButton>
              <input
                checked={newsletter}
                ...
                type="checkbox"
              />
              <label style={{ marginLeft: ".5rem" }} htmlFor="newsletter">
                select if you wish to receive our newsletters
              </label>
            </CheckContainer>
          )}
          <ButtonContainer>
