      ...
      <form method="POST" onSubmit={submitForm}>
        <Container>
          ...
          <DescriptionInput
            ...
            rows={4}
          />
          <HelperText>Users with this role will be able to:</HelperText>
          <PermissionsSection>
            <PermissionBox
              onRemoveClick={onGrantedRemove("read")}
              onAddClick={onGrantedAddition("read")}
              granted={can.read}
              type="Read"
            />
            <PermissionBox
              onRemoveClick={onGrantedRemove("create")}
              onAddClick={onGrantedAddition("create")}
              granted={can.create}
              type="Create"
            />
            <PermissionBox
              onRemoveClick={onGrantedRemove("update")}
              onAddClick={onGrantedAddition("update")}
              granted={can.update}
              type="Update"
            />
            <PermissionBox
              onRemoveClick={onGrantedRemove("delete")}
              onAddClick={onGrantedAddition("delete")}
              granted={can.delete}
              type="Delete"
            />
          </PermissionsSection>
