...
export const DeleteGranted = styled.div`
  ...
  color: ${({ theme }) =>
    lighten(0.1, GetColorType({ theme, type: "danger" }))};
...
export const Container = styled.div<Props>`
  ...
  ${Granted},${AddGranted},${GrantedListStyled} {
    background-color: ${({ theme, type }) =>
      transparentize(0.5, GetColorType({ theme, type }))};
  }
  ${AddGranted} {
    opacity: 0;
  }
  ${GrantedOption} {
    :hover {
      background-color: ${({ theme, type }) =>
        darken(0.3, GetColorType({ theme, type }))};
    }
  }
  :hover {
    border: 3px solid
      ${({ theme, type }) => lighten(0.2, GetColorType({ theme, type }))};
    ${AddGranted} {
      opacity: 1;
    }
