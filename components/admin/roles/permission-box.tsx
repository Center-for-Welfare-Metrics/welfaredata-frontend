...
import {
  ...
  DeleteGranted,
  GrantedListStyled,
  GrantedListBackground,
  GrantedOption,
} from "./permission-box-styled";
...
interface IGrantedList {
  ...
  granted: string[];
  onClick(optionClicked: any): void;
}
...
const GrantedList = ({ onClose, granted, onClick }: IGrantedList) => {
  const onOptionClick = (optionClicked: string) => (event: any) => {
    ...
    <>
      <GrantedListStyled>
        {filterResources(granted).map((access) => (
          <GrantedOption onClick={onOptionClick(access)} key={access}>
            {access}
          </GrantedOption>
        ))}
      </GrantedListStyled>
...
const PermissionBox = ({
  ...
  const addGrantedClicked = (event: any) => {
      ...
      <GrantedContainer>
        {granted.length > 0 ? (
          granted.map((access) => (
            <Granted key={access}>
              {access}
              <DeleteGranted onClick={() => onRemoveClick(access)} />
            </Granted>
          ))
        ) : (
          <HelperMessage>Can't {type} anything</HelperMessage>
        )}
        {filterResources(granted).length !== 0 && (
          <AddGranted type="button" onClick={addGrantedClicked} />
        )}
        {onAddGranted && (
          <GrantedList
            onClick={onAddClick}
