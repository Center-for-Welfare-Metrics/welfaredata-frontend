...
import {
  ...
  AttentionBody,
  FullBackground,
} from "./context-menu-styled";
import {
  Options,
  Option,
  OptionText,
  OptionIcon,
} from "./context-menu-options-styled";
...
const ContextMenu = ({ onClose, isOpen }: IContextMenuComponent) => {
  ...
  const openContextMenu = () => {
    ...
    if (contextMenu.position === "fixed-on-screen") {
      setAxisPosition(true).then(turnContextMenuVisible);
  ...
  return (
    <>
      <FullBackground
        open={isOpen}
        onContextMenu={innerContextMenu}
        onClick={onClose}
      />
      <Container
        open={isOpen}
        ...
        ref={containerRef}
      >
        {contextMenu.type === "processogram" && <ProcessogramMenu />}
        {contextMenu.type === "options" && (
          <Options>
            {contextMenu.options?.map(({ text, onClick, icon, type }) => (
              <Option
                type={type}
                onClick={() => onClick(contextMenu.optionTarget)}
                key={text}
              >
                <OptionIcon
        ...
        {contextMenu.type === "none" && (
          <AttentionBody>
            <>
              if you perform this action on a element of the processogram, you
              will be able to obtain more information, links to bibliography,
              videos - and even provide us with feedback about that element!
            </>
          </AttentionBody>
        )}
      </Container>
